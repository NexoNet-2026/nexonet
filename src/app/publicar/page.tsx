...
const TIPOS_NEXO = [
  { id:"anuncio",  emoji:"📣", titulo:"Anuncio",      desc:"Vendé, comprá o permutá lo que quieras",          color:"#d4a017", bg:"linear-gradient(135deg,#1a2a3a,#2d3d50)", border:"#d4a017", subtipos:null, tablaRubros:"rubros", tablaSubrubros:"subrubros" },
  ...
  ], tablaRubros:null, tablaSubrubros:null },
  { id:"empresa",  emoji:"🏢", titulo:"Empresa",       desc:"Perfil comercial con 50 anuncios incluidos",      color:"#c0392b", bg:"linear-gradient(135deg,#2c1a1a,#4a2020)", border:"#c0392b", subtipos:null, tablaRubros:"empresa_rubros", tablaSubrubros:"empresa_subrubros" },
  { id:"servicio", emoji:"🛠️", titulo:"Servicio",      desc:"Mostrá lo que ofrecés con videos y portfolio",   color:"#27ae60", bg:"linear-gradient(135deg,#1a2e1a,#1e4a2a)", border:"#27ae60", subtipos:null, tablaRubros:"servicio_rubros", tablaSubrubros:"servicio_subrubros" },
  { id:"trabajo",  emoji:"💼", titulo:"Busco Trabajo", desc:"Ofrecé tus habilidades y adjuntá tu CV",          color:"#8e44ad", bg:"linear-gradient(135deg,#1e1a2e,#2e1a4a)", border:"#8e44ad", subtipos:null, tablaRubros:"trabajo_rubros", tablaSubrubros:"trabajo_subrubros" },
];
...
export default function PublicarSelector() {
  ...
  const seleccionarTipo = async (tipoId: string) => {
    ...
    // Grupos → mostrar subtipos
    if (tipo.subtipos) { setSeleccionado(tipoId); return; }
  ...
  const seleccionarRubro = async (rubro: any) => {
    ...
    setCargando(true);
    const fk = seleccionado === "anuncio" ? "rubro_id" : "rubro_id";
  ...
  return (
    <main style={{ paddingTop:"95px", paddingBottom:"160px", background:"#0f1923", minHeight:"100vh", fontFamily:"'Nunito',sans-serif" }}>
      ...
      {/* SUBTIPOS DE GRUPO */}
      {paso === "tipo" && seleccionado && tipoActual?.subtipos && (
        <div style={{ padding:"0 16px", opacity:animando?0:1, transition:"opacity .2s" }}>
          <button onClick={volver} style={{ background:"rgba(255,255,255,0.08)", border:"1px solid rgba(255,255,255,0.15)", borderRadius:"10px", padding:"8px 14px", color:"rgba(255,255,255,0.6)", fontSize:"13px", fontWeight:700, cursor:"pointer", fontFamily:"'Nunito',sans-serif", marginBottom:"16px" }}>← Volver</button>
          <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:"20px", color:"#3a7bd5", letterSpacing:"1px", marginBottom:"14px" }}>👥 ¿Qué tipo de grupo?</div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px" }}>
            {tipoActual.subtipos.map(s => (
              <button key={s.id} onClick={() => elegir("grupo", s.id)}
