...
export default function ChatNexoPage() {
  const params  = useParams();
  const router  = useRouter();
  // URL: /chat/nexo/[nexo_id]/[usuario_id]
  const nexoId     = params.nexo_id as string;
  ...
  const cargar = async () => {
    ...
    // Cargar nexo
    const { data: nx } = await supabase.from("nexos").select("id,titulo,avatar_url,tipo").eq("id", nexoId).single();
    if (nx) setNexo(nx);
  ...
  return (
    <main style={{ paddingTop:"95px", paddingBottom:"0", background:"#f4f4f2", minHeight:"100vh", fontFamily:"'Nunito',sans-serif", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ position:"fixed", top:"100px", left:0, right:0, zIndex:90, background:"linear-gradient(135deg,#1a2a3a,#243b55)", padding:"10px 16px", display:"flex", alignItems:"center", gap:"12px", boxShadow:"0 2px 12px rgba(0,0,0,0.3)" }}>
        ...
        {/* Chip nexo */}
        {nexo && (
          <button onClick={()=>router.push(`/nexo/${nexoId}`)} style={{ background:"rgba(212,160,23,0.2)", border:"1px solid rgba(212,160,23,0.4)", borderRadius:"10px", padding:"4px 10px", cursor:"pointer", maxWidth:"120px" }}>
            <div style={{ fontSize:"10px", fontWeight:800, color:"#d4a017", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{nexo.titulo}</div>
            <div style={{ fontSize:"10px", color:"#8a9aaa", fontWeight:600 }}>{nexo.tipo}</div>
          </button>
        )}
      </div>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"170px 16px 210px", display:"flex", flexDirection:"column", gap:"4px", background:"#eae6df" }}>
        ...
        {nexo && otroUser && (
          <div style={{ background:"rgba(255,255,255,0.8)", borderRadius:"12px", padding:"12px 16px", margin:"0 0 8px", textAlign:"center", border:"1px solid rgba(212,160,23,0.2)" }}>
            ...
            <div style={{ fontSize:"11px", color:"#9a9a9a", fontWeight:600 }}>sobre el nexo
              <a href={`/nexo/${nexoId}`} style={{ color:"#d4a017", fontWeight:800, textDecoration:"none" }}> {nexo.titulo} →</a>
            </div>
          </div>
        )}
        {mensajes.length === 0 && (
          <div style={{ textAlign:"center", padding:"40px 20px", color:"#9a9a9a" }}>
            ...
            <div style={{ fontSize:"12px", marginTop:"6px", color:"#888" }}>Este chat es privado entre vos y el creador del nexo.</div>
          </div>
        )}
