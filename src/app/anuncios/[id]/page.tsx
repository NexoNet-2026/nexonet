...
export default function AnuncioDetalle() {
  ...
  const ejecutarConexion = async (tipoBit: "nexo"|"promo"|"free" = "nexo") => {
    ...
    if (anuData) {
      ...
      if (tipoBit === "nexo") {
        ...
        const nuevo = bitsPromo - 1;
        await supabase.from("usuarios").update({ bits_promo: nuevo, bits_promo_gastados: nuevo }).eq("id", session.user.id);
        setBitsPromo(nuevo);
      } else {
        const nuevo = bitsFree - 1;
        await supabase.from("usuarios").update({ bits_free: nuevo, bits_free_gastados: nuevo }).eq("id", session.user.id);
        setBitsFree(nuevo);
      }
    }
    setConectando(false);
