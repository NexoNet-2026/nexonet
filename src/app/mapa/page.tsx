  ...
  useEffect(() => {
    const cargar = async () => {
      const { data } = await supabase
        .from("anuncios")
        .select("id, titulo, precio, moneda, imagenes, flash, ciudad, provincia, lat, lng")
        .eq("estado", "activo")
        .not("lat", "is", null)
        .not("lng", "is", null);
      ...
      <div style={{ position: "fixed", top: "150px", left: 0, right: 0, bottom: "130px" }}>
        {loading ? (
          <div style={{
            ...
            height: "100%", background: "#f4f4f2", fontSize: "14px", fontWeight: 700, color: "#9a9a9a",
          }}>
            Cargando anuncios...
          </div>
        ) : (
        ...
        {anuncioSeleccionado && (
          <div style={{
            ...
            <div style={{
              ...
              flexShrink: 0, background: "#f4f4f2", display: "flex", alignItems: "center", justifyContent: "center",
            }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: "11px", color: "#9a9a9a", fontWeight: 600 }}>
                📍 {anuncioSeleccionado.ciudad}{anuncioSeleccionado.provincia ? `, ${anuncioSeleccionado.provincia}` : ""}
              </div>
            </div>
            {anuncioSeleccionado.flash && (
              <div style={{
                background: "#d4a017", color: "#1a2a3a", fontSize: "9px", fontWeight: 900,
                padding: "3px 7px", borderRadius: "8px", textTransform: "uppercase", alignSelf: "flex-start",
              }}>⚡Flash</div>
            )}
            <div style={{ display: "flex", flexDirection: "column", gap: "4px", alignSelf: "flex-start" }}>
              <button onClick={() => setAnuncioSeleccionado(null)} style={{
                background: "none", border: "none", fontSize: "16px", cursor: "pointer", color: "#9a9a9a",
              }}>✕</button>
              <a href={`/anuncios/${anuncioSeleccionado.id}`} style={{
                background: "#d4a017", color: "#1a2a3a", border: "none", borderRadius: "8px",
                padding: "4px 8px", fontSize: "11px", fontWeight: 800, textDecoration: "none", textAlign: "center",
              }}>Ver →</a>
            </div>
          </div>
        )}
        ...
        <div style={{
          ...
          borderTop: "1px solid #e8e8e6", zIndex: 999,
        }}>
