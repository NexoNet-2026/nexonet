  ...
  const cargar = async () => {
    ...
    if (data.usuario_id) {
      const { data: u } = await supabase
        .from("usuarios")
        .select("nombre_usuario, nombre, apellido, codigo, plan, whatsapp, telefono, whatsapp_empresa, direccion, ciudad, provincia, barrio, direccion_empresa, ciudad_empresa, provincia_empresa, barrio_empresa, vis_personal, vis_empresa, insignia_logro, es_bot")
                  ...
                  {!soloChatInterno && (() => {
                    ...
                    return (
                      <button onClick={() => {
                        ...
                        if (usuario?.es_bot && anuncio?.link_externo) { window.open(anuncio.link_externo, '_blank'); return; }
                        setPopupMensaje(true);
